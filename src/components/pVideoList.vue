      ...
      <div class="fileList clearfix">
        <ul class="clearfix">
          <li v-for="(item, index) in videoList">
            <router-link tag="a" class="" :to="'/article/' + currentP.name + '/video/' + item.mId" :title="item.title" target="_blank">
              <img class="fileImg" :src="getHeadPortraitSrc2(item.video.videoImg.url)" alt="" v-if="item.video.videoImg.url.length !== 0"/>
